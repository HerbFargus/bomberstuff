...
public static class ColorRemapper
{
	...
	/// </summary>
	/// <param name="srcState">
	/// the sequence state is frame is to be remapped
	/// </param>
	/// <param name="destState">
	/// The destination state in which the new information should be saved
	/// </param>
	/// <param name="changes">
	...
	/// </remarks>
	public static void Remap(SeqState destState, SeqState srcState, ColorRemapInfo changes)
	{
		BitmapBuilder dest = destState.BitmapBuilder;
		BitmapBuilder src = srcState.BitmapBuilder;
		...
		switch (src.BitsPerPixel)
		{
			...
			case 16:
			{
				...
				Array.Copy(src.BitmapData, dest.BitmapData, src.DataLocation);
		...
		destState.SetKeyColor(newRawKeyColor, newKeyColor);
	}
